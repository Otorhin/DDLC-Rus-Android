        ...
        if not persistent.autoload or not main_menu:
            ...
            if main_menu:
                ...
                if persistent.playthrough == 1:
                    textbutton _("ŔŗñĮ¼»ŧþŀÂŻŕěōì«") action If(persistent.playername, true=Start(), false=Show(screen="name_input", message="Введите ваше имя", ok_action=Function(FinishEnterName)))
                else:
                    textbutton _("Новая игра") action If(persistent.playername, true=Start(), false=Show(screen="name_input", message="Введите ваше имя", ok_action=Function(FinishEnterName)))
            ...
            if renpy.variant("pc"):
                ...
                textbutton _("Помощь") action [Help("README.html"), Show(screen="dialog", message="The help file has been opened in your browser.", ok_action=Hide("dialog"))]
    ...
    def FileActionMod(name, page=None, **kwargs):
        if persistent.playthrough == 1 and not persistent.deleted_saves and renpy.current_screen().screen_name[0] == "load" and FileLoadable(name):
            return Show(screen="dialog", message="File error: \"characters/sayori.chr\"\n\nФайл не найден или поврежден.",
                ok_action=Show(screen="dialog", message="Файл сохранения поврежден. Начните новую игру.", ok_action=Function(renpy.full_restart, label="start")))
        elif persistent.playthrough == 3 and renpy.current_screen().screen_name[0] == "save":
            return Show(screen="dialog", message="Данный файл сохранения больше не доступен.", ok_action=Hide("dialog"))
        else:
